...
create table Incident
  (
    ...
    type varchar(20),
    primary key (id),
    foreign key (type) references Disaster(name)
...
create table Report
  (
    ...
    foreign key (govn_ssn) references Government_Representative(ssn) on delete no action on update no action,
    foreign key (citizen_ssn) references Citizen(ssn) on delete set null on update cascade
  );

create table Discussion
  (
    dics_id int auto_increment,
    answer text,
...
create table Casualty_Incident
  (
    ...
    foreign key (incident_id) references Incident(id) on delete cascade on update cascade,
    foreign key (casualty_ssn) references Casualty(ssn) on delete cascade on update cascade
  );
...
create table Criminal_HM_Incident
  (
    ...
    foreign key (criminal_ssn) references Criminal(ssn) on delete cascade on update cascade,
    foreign key (hm_incident_id) references Human_Made(id) on delete cascade on update cascade
  );
